...
def isCodifiedVU(para):
    ...
    for line in noVUID:
        ...
        isIf = line.startswith('if ')
        isFor = line.startswith('for ')
        isWhile = line.startswith('while ')
        isAssign = assignmentPattern.search(line) is not None
        isRequire = line.startswith('require(')

        return isIf or isFor or isWhile or isAssign or isRequire
...
def createFeatureMap(registry, aliasMap):
    """Create a map from feature names (i.e. members of Vk*FeaturesSUFFIX
    structs) to Vulkan versions and extension names.  VUs frequently
    references feature names, and this map facilitates that."""

    # First, go over Vulkan versions (<feature> tags) and extensions and
    # see which Vk*Features structs belong to which version/extensions.
    featureStructMap = {}

    versionAndExtensionList = [(info.versionNumber, info.elem) for _, info
    ...
    for versionAndExtension, elem in versionAndExtensionList:
        for struct in elem.findall('.//type'):
            ...
            if featuresStructPattern.match(name) is not None:
                if name not in featureStructMap:
                    featureStructMap[name] = set()
                featureStructMap[name].add(versionAndExtension)

    # Then go over the members of each Vk*Features struct and map their members
    ...
    for struct, versionsAndExtensions in featureStructMap.items():
        ...
        for member in info.getMembers():
            ...
            if name not in featureMap:
                featureMap[name] = set()
            featureMap[name] = featureMap[name].union(versionsAndExtensions)
...
class VuOutputStyler:
    ...
    def __init__(self, registry, filename, fileline):
        ...
        self.featureList = createFeatureMap(self.registry, aliasMap).keys()
...
class VuTypeExtractor:
    ...
    def __init__(self, registry, api):
        ...
        self.featureMap = createFeatureMap(self.registry, self.aliasMap)
...
class VuVerifier(ast.NodeVisitor):
    ...
    def visit_Assign(self, node):
        ...
        elif lhs in FUNC_BUILTINS.keys() or lhs in ATTR_BUILTINS.keys():
