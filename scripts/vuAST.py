...
class VuTypeExtractor:
    ...
    def createAliasMap(self):
        # Look at all entries that can be aliased:
        aliasMap = {}

        # Types
        for type_elem in self.registry.reg.findall('types/type'):
        ...
        # Commands
        for cmd in self.registry.reg.findall('commands/command'):
            alias = cmd.get('alias')
            if not alias:
                continue
    ...
    def createEnumValueMap(self):
        valueMap = {}

        for enums in self.registry.reg.findall('enums'):
            name = enums.get('name')
            assert(name not in self.aliasMap)
            for enum in enums.findall('enum'):
                value = enum.get('name')
...
def formatStrippedVU(vuParagraph, apiName, filename, fileline, strippedIndent, registry, macros):
