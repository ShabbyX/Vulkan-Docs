...
from vuAST import createAliasMap, createFeatureMap
...
def verify_format(registry, versions, extensions, command, expectedResponse):
    ...
    response = processFormatCommand(command.splitlines(), registry, versions, extensions, featureMap)
    response = '\n'.join(response)
...
"""
    verify_versions(registry, command, ['1.0'], [])

    command = """1.0 1.1
VK_KHR_maintenance1 VK_EXT_dynamic_rendering"""
    verify_versions(registry, command, ['1.0', '1.1'], ['VK_KHR_maintenance1', 'VK_EXT_dynamic_rendering'])
...
FORMAT-VU-FAIL"""
    ...
    verify_format(registry, ['1.0'], [], command, expect);
...
&nbsp;&nbsp;[vu-builtin]##<<vu-builtin-require,require>>##&lpar;stencilResolveMode [vu-operator]##==## ename:VK_RESOLVE_MODE_MAX_BIT&rpar;#
FORMAT-VU-SUCCESS"""
...
FORMAT-VU-SUCCESS"""
    ...
    verify_format(registry, ['1.0', '1.1'], [], command, expect);
...
FORMAT-VU-ELIMINATED"""
    ...
    verify_format(registry, ['1.0', '1.1', '1.2', '1.3'], ['VK_NV_shader_subgroup_partitioned', 'VK_NV_compute_shader_derivatives'], command, expect);
...
if not has_feature:
    ...
    expect = """FORMAT-VU
[vu]#[vu-builtin]##<<vu-builtin-require,require>>##&lpar;representativeFragmentTestEnable [vu-operator]##==## ename:VK_FALSE&rpar;#
FORMAT-VU-SUCCESS"""
...
if not has_feature:
    require(representativeFragmentTestEnable == VK_FALSE)"""
    expect = """FORMAT-VU
[vu]#has_feature [vu-operator]##=## [vu-builtin]##<<vu-builtin-is_feature_enabled,is_feature_enabled>>##&lpar;<<features-representativeFragmentTest,representativeFragmentTest>>&rpar; +
[vu-keyword]##if## [vu-operator]##not## has_feature: +
&nbsp;&nbsp;[vu-builtin]##<<vu-builtin-require,require>>##&lpar;representativeFragmentTestEnable [vu-operator]##==## ename:VK_FALSE&rpar;#
FORMAT-VU-SUCCESS"""

    verify_format(registry, ['1.0', '1.1', '1.2', '1.3'], ['VK_NV_shader_subgroup_partitioned', 'VK_NV_representative_fragment_test', 'VK_NV_compute_shader_derivatives'], command, expect);
