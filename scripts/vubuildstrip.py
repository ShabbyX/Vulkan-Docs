...
class VuBuildStrip(ast.NodeVisitor):
    ...
    are not being built."""
    def __init__(self, vu, versions, extensions, featureMap):
        self.vu = vu
        """The original AST."""

        self.versions = versions
        """The versions being included in the build (e.g. ['1.0', '1.1'])."""
    ...
    def stripIsVersion(self, node):
        ...
        version = str(node.args[0].value) + '.' + str(node.args[1].value)
    ...
    def stripIsFeatureEnabled(self, node):
        ...
        assert(len(node.args) == 1)
        assert(isinstance(node.args[0], ast.Name))
        assert(node.args[0].id in self.featureMap)
        ...
        def hasExtension(extension, extensions):
          return extension.startswith('VK_') and extension.lower() in extensions
        def hasVersion(version, versions):
          return not version.startswith('VK_') and version in versions

        hasFeature = any([hasExtension(cond, self.extensions) or
                          hasVersion(cond, self.versions)
                          for cond in self.featureMap[node.args[0].id]])
    ...
    def visit_While(self, node):
        # Strip the condition; if empty, remove the entire loop.  With the
        # current restriction of immutable variables, this is currently a
        # no-op.
        test = self.visit(node.test)
        if test is None:
            return None

        # Warn about constants in loops
        if self.isConstantBool(test):
            self.warn('while condition evaluates to constant in this build.')
...
class VuStripDeadCode(ast.NodeTransformer):
    ...
    def visit_While(self, node):
        ...
        return ast.While(node.test, body, [])

def stripVUForBuild(vu, versions, extensions, featureMap):
    """Strip a VU to remove references to Vulkan versions and extensions that
    are not being built.

    Takes the VU AST after macro substitution, the list of versions that are
    being built (such as ['1.0', '1.1', '1.2']) and the list of extensions.
    Note that normally the latest version (e.g. 1.2) implies the earlier
    versions (e.g. 1.1), but the Makefile technically allows building a later
    but not earlier version.

    The version and extension lists are used to turn is_ext_enabled() and
    is_feature_enabled() to False when not being included in build.  is_version
    is turned to True or False such that it is never visible in the spec.

    Then, const-propagation is applied to the VU and the result is returned.
    """

    buildstrip = VuBuildStrip(vu, versions, extensions, featureMap)
