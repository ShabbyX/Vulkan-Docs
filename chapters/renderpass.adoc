...
ifndef::VK_VERSION_1_1,VK_KHR_device_group[]
  ...
  * [[VUID-VkRenderingInfo-imageView-06075]]
  ...
  * [[VUID-VkRenderingInfo-imageView-06076]]
...
ifdef::VK_VERSION_1_1,VK_KHR_device_group[]
  ...
  * [[VUID-VkRenderingInfo-pNext-07815]]
    If the pname:pNext chain does not contain
    slink:VkDeviceGroupRenderPassBeginInfo or its
    pname:deviceRenderAreaCount member is equal to 0, the sum of
    pname:renderArea.extent.width and pname:renderArea.offset.x must: be
  ...
  * [[VUID-VkRenderingInfo-pNext-07816]]
    ...
    slink:VkDeviceGroupRenderPassBeginInfo or its
    pname:deviceRenderAreaCount member is equal to 0, the sum of
    pname:renderArea.extent.width and pname:renderArea.offset.y must: be
    less than or equal to
...
pname:resolveImageView is not dlink:VK_NULL_HANDLE, values in
...
slink:VkRenderingInfo::pname:pDepthAttachment, and the stencil aspect if it
is used as pname:pStencilAttachment).
...
ifndef::VK_EXT_multisampled_render_to_single_sampled[]
  ...
  * [[VUID-VkRenderingAttachmentInfo-imageView-06860]]
    If pname:imageView is not dlink:VK_NULL_HANDLE and pname:resolveMode is
    not ename:VK_RESOLVE_MODE_NONE, pname:resolveImageView must: not be
...
ifdef::VK_EXT_multisampled_render_to_single_sampled[]
  * [[VUID-VkRenderingAttachmentInfo-imageView-06861]]
    ...
    ename:VK_RESOLVE_MODE_NONE, and the pname:pNext chain of
    slink:VkRenderingInfo does not includes a
    ...
    ename:VK_TRUE, pname:imageView must: not have a sample count of
  ...
  * [[VUID-VkRenderingAttachmentInfo-imageView-06862]]
    ...
    slink:VkMultisampledRenderToSingleSampledInfoEXT structure with the
    pname:multisampledRenderToSingleSampledEnable field equal to
    ename:VK_TRUE, pname:resolveImageView must: not be dlink:VK_NULL_HANDLE
...
include::{chapters}/commonvalidity/attachment_description_common.adoc[]
  * [[VUID-VkAttachmentDescription-format-06700]]
    If pname:format includes a stencil aspect and pname:stencilLoadOp is
...
A subpass may access the same subresource for both a color or depth/stencil
attachment and as an input resource in one of the following ways:

  * As an input attachment
ifdef::VK_EXT_attachment_feedback_loop_layout[]
  * As a sampled image or image resource in
    ename:VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL_EXT layout in a
    pipeline with either the
    ename:VK_PIPELINE_CREATE_COLOR_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT or
    ename:VK_PIPELINE_CREATE_DEPTH_STENCIL_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT
    bits set in a render pass that contains dependencies with the
    ename:VK_DEPENDENCY_FEEDBACK_LOOP_BIT_EXT bit set
endif::VK_EXT_attachment_feedback_loop_layout[]

In these situations, writes via the color or depth/stencil attachment are
not automatically made visible to reads via the input resource, causing a
_feedback loop_, except in any of the following conditions:

  * If the color components or depth/stencil components read are mutually
    exclusive with the components written by the color or depth/stencil
    attachments, then there is no feedback loop.
...
Rendering within a subpass containing a feedback loop creates a
<<memory-model-access-data-race, data race>>, except in the following cases:
  ...
  * If a memory dependency is inserted between when the attachment is
    ...
    subpass.
ifdef::VK_EXT_rasterization_order_attachment_access,VK_ARM_rasterization_order_attachment_access[]
  * If the attachment is used as color and input attachment, and the
    pipeline performing the read was created with
    ename:VK_PIPELINE_COLOR_BLEND_STATE_CREATE_RASTERIZATION_ORDER_ATTACHMENT_ACCESS_BIT_EXT
    included in the pname:flags member of the pname:pColorBlendState member
    of its slink:VkGraphicsPipelineCreateInfo.
    This creates a framebuffer-local memory dependency for each fragment
    generated by draw commands using this pipeline with the following
    properties:
  ** The first <<synchronization-dependencies-scopes, synchronization
     scope>> includes the
     ename:VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT pipeline stage
     executed by all previous fragments (as defined by
  ...
  * If the attachment is used as depth/stencil and input attachment, and the
    pipeline performs a read of the depth aspect and was created with
    ename:VK_PIPELINE_DEPTH_STENCIL_STATE_CREATE_RASTERIZATION_ORDER_ATTACHMENT_DEPTH_ACCESS_BIT_EXT
    included in the pname:flags member of the pname:pDepthStencilState
    member of its slink:VkGraphicsPipelineCreateInfo.
    This creates a memory dependency for each fragment generated by draw
    commands using this pipeline with the following properties:
  ** The first <<synchronization-dependencies-scopes, synchronization
  ...
  * If the attachment is used as depth/stencil and input attachment, and the
    pipeline performs a read of the stencil aspect and was created with
    ename:VK_PIPELINE_DEPTH_STENCIL_STATE_CREATE_RASTERIZATION_ORDER_ATTACHMENT_STENCIL_ACCESS_BIT_EXT
    included in the pname:flags member of the pname:pDepthStencilState
    member of its slink:VkGraphicsPipelineCreateInfo.
    This creates a memory dependency for each fragment generated by draw
...
.Note
====
Writes inside of a feedback loop for images in the
ename:VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL_EXT image layout only
avoid a <<memory-model-access-data-race, data race>> in the following
conditions:

  * The fragment (p,q) is sampling the texel at (p,q), with a
    <<synchronization-pipeline-barriers-subpass-self-dependencies, subpass
    self-dependency>>.
  * Reading/sampling from a region not currently being written to the image
    as a color or depth/stencil attachment in the current subpass (disjoint
    read/write regions).
====
endif::VK_EXT_attachment_feedback_loop_layout[]
...
****
  ...
  * [[VUID-VkRenderPassCreateInfo2-attachment-03051]]
    ...
    pname:pSubpasses is not ename:VK_ATTACHMENT_UNUSED, then it must: be
    less than pname:attachmentCount
ifdef::VK_EXT_fragment_density_map[]
...
ifdef::VK_VERSION_1_2,VK_KHR_separate_depth_stencil_layouts[]
  * [[VUID-VkRenderPassCreateInfo2-attachment-06244]]
    ...
    have a pname:format that includes both depth and stencil aspects
  * [[VUID-VkRenderPassCreateInfo2-attachment-06245]]
    ...
    pname:format that includes only a stencil aspect
  * [[VUID-VkRenderPassCreateInfo2-attachment-06246]]
    ...
    pname:pAttachments with an index equal to pname:attachment must: not
    have a pname:format that includes only a stencil aspect
...
ifdef::VK_VERSION_1_2,VK_KHR_separate_depth_stencil_layouts[]
  * [[VUID-VkAttachmentDescription2-pNext-06704]]
    ...
    includes a stencil aspect, and pname:stencilLoadOp is
    ename:VK_ATTACHMENT_LOAD_OP_LOAD, then pname:initialLayout must: not be
    ename:VK_IMAGE_LAYOUT_UNDEFINED
  * [[VUID-VkAttachmentDescription2-pNext-06705]]
    If the pname:pNext chain does includes a
    slink:VkAttachmentDescriptionStencilLayout structure, pname:format
    includes a stencil aspect, and pname:stencilLoadOp is
  ...
  * [[VUID-VkAttachmentDescription2-format-06247]]
    ...
    only includes a stencil aspect, pname:initialLayout must: not be
    ename:VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL or
    ename:VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL
  * [[VUID-VkAttachmentDescription2-format-06248]]
    If the pname:pNext chain does not include a
    slink:VkAttachmentDescriptionStencilLayout structure and pname:format
    only includes a stencil aspect, pname:finalLayout must: not be
    ename:VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL or
    ename:VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL
endif::VK_VERSION_1_2,VK_KHR_separate_depth_stencil_layouts[]
****
...
ifdef::VK_VERSION_1_2,VK_KHR_separate_depth_stencil_layouts[]
  * [[VUID-VkSubpassDescription2-attachment-06251]]
    ...
    slink:VkAttachmentDescriptionStencilLayout structure, the pname:layout
...
If pname:pDepthStencilResolveAttachment is `NULL`, or if its attachment
index is ename:VK_ATTACHMENT_UNUSED, it indicates that no depth/stencil
...
****
  ...
  * [[VUID-VkSubpassDescriptionDepthStencilResolve-pDepthStencilResolveAttachment-03181]]
    ...
    component with the same number of bits and numerical type
...
image subresources that are used as attachments
ifdef::VK_EXT_attachment_feedback_loop_layout[]
that are not in ename:VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL_EXT
layout
endif::VK_EXT_attachment_feedback_loop_layout[]
in a render pass instance happen-before or happen-after the render pass
instance.
...
.Note
====
An image subresource can be used as read-only as both an attachment and a
non-attachment during a render pass instance, but care must still be taken
to avoid data races with load/store operations and layout transitions.
The simplest way to achieve this is to keep the non-attachment and
attachment accesses within the same subpass, or to avoid layout transitions
and load/store operations that perform writes.
====
...
ifdef::VK_VERSION_1_1,VK_KHR_multiview[]
  ...
  * [[VUID-VkFramebufferCreateInfo-renderPass-04546]]
    ...
    includes ename:VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT, the pname:layerCount
    member of any element of the pname:pAttachmentImageInfos member of a
    slink:VkFramebufferAttachmentsCreateInfo structure included in the
    pname:pNext chain used as an input, color, resolve, or depth/stencil
